...
install.packages("SemiPar", repos = "https://cloud.r-project.org/")
library(SemiPar)
library(modelr)
library(mgcv)
...
```{r}
linear_mode = lm(armc~weight, data = child_df)
...
cv_res_df = 
  ...
  mutate(
    ...
    osl_mod = map(train, \(x) lm(armc~weight + weight_ch7, data = x)), 
    smooth_mod = map(train, \(x) gam(armc ~s(weight)))
  ) %>% 
  mutate(
    rmse_linear = map2_dbl(linear_mod, test, rmse)
  )
